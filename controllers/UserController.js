...
module.exports = {
    ...
    async getPopulatedFavorites(req, res) {
        try {
            const favorites = await db.User.findOne({ _id: req.params.id }, { favorites: 1, _id: 0 }).populate({ path: 'favorites', populate: { path: 'character soulsetMain soulsetSub', populate: { path: 'tags' } } });
            res.json(favorites);
        } catch (err) {
